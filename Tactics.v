...
(*Search rev.*) apply rev_involutive. Qed.  
  (** [] *)
...
Example trans_eq_exercise : forall (n m o p : nat),
     ...
     (n + p) = (minustwo o).
Proof.
  intros n m o p eq eq2.
apply trans_eq with m. apply eq2. apply eq.
 Qed.
  (** [] *)
...
Theorem plus_n_n_injective : forall n m,
  ...
  n = m.
Proof.
  (*Search plus*)intros n. induction n as [| n' IHn'].
  - simpl. destruct m.
    + simpl. reflexivity.
    + simpl. intros contra. discriminate contra. 
  - simpl. destruct m.
    + simpl. intros contra. discriminate contra.
    + simpl. intros eq. rewrite <- plus_n_Sm in eq.  rewrite <- plus_n_Sm in eq. 
      injection eq as eqi. 
      apply IHn' in eqi.
...
Proof.
  ...
  - destruct n.
    + simpl. discriminate.  
    + simpl. intros eq. injection eq as eqi. apply IHl in eqi. apply eqi.
Qed. 
  (** [] *)
...
Proof.
  intros X Y l.
  induction l as [| h t IHl];intros l1 l2 eq.
  -  inversion eq. reflexivity.
  -  inversion eq. destruct h. destruct (split t). simpl in H0. inversion H0. simpl. apply f_equal. apply IHl. reflexivity.
Qed.
  (** [] *)
